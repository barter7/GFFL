...
import PlayerCell, { SnapCell } from "@/components/PlayerCell";
import s from "@/components/depth.module.css";
  ...
  function Pod({ team: t, grp }: { team: string; grp: string }) {
    ...
    return (
      <section className={s.pod}>
        <div className={`${s.podHead} ${posClass(s, "head", grp)}`}>
          {mode === "team" ? (
            <span className={`${s.grp} ${posClass(s, "ink", grp)}`}>{grp}</span>
          ) : (
            <>
              <span className={s.team}>{t}</span>
              <span className={`${s.grp} ${posClass(s, "ink", grp)}`}>
                {grp}
              </span>
            </>
          )}
          ...
          <table className={s.tbl}>
            <tbody>
              {din.map((r) => (
                <tr
                  key={r.gsis_id}
                  className={`${s.posRail} ${posClass(s, "rail", grp)} ${
                    r.status !== "RET" ? s.rowNew : ""
                  }`}
                >
              ...
              {dout.map((r) => (
                <tr
                  key={r.gsis_id}
                  className={`${s.posRail} ${posClass(s, "rail", grp)} ${s.rowOut}`}
                >
              ...
              <table className={s.tbl}>
                ...
                <tbody>
                  {boardRows.map((r, i) => {
                    ...
                    const d = D.depthById[r.gsis_id];
                    const val =
                      key === "adp" ? r.adp : key === "r2" ? r.ecr2 : r.ecr1;
                    return (
                      <tr
                        key={r.gsis_id}
                        className={`${s.posRail} ${posClass(s, "rail", r.pos)} ${
                          d?.status && d.status !== "RET" ? s.rowNew : ""
                        }`}
                      >
                        <td className={`${s.ovr} ${s.mono}`}>{i + 1}</td>
                        <td className={s.posRankCell}>
                          <span
                            className={`${s.pill} ${posClass(s, "pill", r.pos)} ${s.mono}`}
                          >
                            {r.pos}
                            {posn[r.pos]}
                          </span>
                        ...
                        <PlayerCell
                          ...
                          extra={
                            <>
                              ...
                              {d?.slot ? (
                                <span className={s.tm} title="depth-chart slot">
                                  {r.pos}
                                  {d.slot}
                                </span>
                              ) : null}
  ...
  return (
    <>
      <div className="sub" style={{ marginTop: 10 }}>
        ...
        <span className="stamp"> Generated {D.meta.generated}.</span>
      </div>
      ...
      <footer>
        ...
        <p>
          Depth order is the latest 2026 camp snapshot — a snapshot, not a Week
          1 guarantee. <b>NEW</b> = on another team in 2025 (the roster diff
          can&rsquo;t separate a signing from a trade); <b>ROOKIE</b> shows its
          draft slot, or UDFA. Departures are 2025 players with 250+ snaps no
          longer on the roster. Bars are 2025 offensive + defensive snaps.
        </p>
        <p>
          <b>Availability:</b> official weekly injury reports don&rsquo;t exist
          until Week 1, so preseason status comes from the roster —{" "}
          <b>RESERVE</b> = IR/PUP/NFI, <b>NOT SIGNED</b> = unsigned or
          unreported draft pick (verified: that code appears on 17 rookies and 0
          veterans, so it is a contract status, not an injury). The red
          durability note is weeks listed <i>Out</i> for injury in 2025,
          excluding rest and coach&rsquo;s-decision rows.
        </p>
        <p>
          <b>Fantasy stock:</b> ▲ helps, ▼ hurts, ◆ changes the picture without
          a clear sign — click any chip for the evidence behind it. Chips cover
          QB/RB/WR/TE only and are computed, not editorial: <b>DRAFT</b> = a
          round 1–3 pick at his own position; <b>SIGNED</b>/<b>VACATED</b> = a
          same-position arrival or departure, shown with that player&rsquo;s
          actual 2025 line, because &ldquo;they signed a receiver&rdquo; means
          nothing until you know whether he saw 30 targets or 130; <b>TGTS</b>/
          <b>CARR</b> = the share of the team&rsquo;s 2025 targets or carries
          now held by players off the roster; <b>HC</b>/<b>OC</b>/<b>QB</b> =
          turnover above him; <b>PACE</b> = 2025 seconds per snap at least 1.5s
          off the league mean; <b>AVAIL</b>/<b>INJ</b> = his own roster status
          or 5+ weeks lost in 2025. These are volume-context flags, not
          projections.
        </p>
        <p>
          ...
          <code>gsis_id</code> (99% matched; the 2026 rookie class isn&rsquo;t
          in that crosswalk yet, so those rows fall back to a name+position
          match against our own roster and say so).{" "}
        ...
        <p>
          ...
          {D.meta.has_pff ? (
            ...
            <>
              {" "}
              <b>PFF grades:</b> not available — PFF is proprietary and appears
              in no nflverse dataset, so these lines use nflverse production
              instead. Offensive line is where that gap bites hardest.
            </>
          )}
        </p>
      </footer>
