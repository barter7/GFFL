...
build time. Refreshing the model therefore means: rebuild the
model caches, run `export_site.R`, commit the JSON, push. The
standalone artifact HTML in `web/` stays as the no-build preview
